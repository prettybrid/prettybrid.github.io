...
function render(){
  ...
  if(viewMode==="grid"){
    ...
    for(i=0;i<pieces.length;i++){
      ...
      del=adminMode?"<div style='display:flex;gap:0.5rem;margin-top:0.75rem;'><button class='bedit' data-idx='"+i+"' style='font-family:Courier Prime,monospace;font-size:0.5rem;letter-spacing:0.15em;text-transform:uppercase;padding:0.5rem 0.75rem;background:transparent;color:#B8934A;border:1px solid #B8934A;cursor:pointer;border-radius:2px;flex:1;'>Edit</button><button class='bdel' data-idx='"+i+"' style='font-family:Courier Prime,monospace;font-size:0.5rem;letter-spacing:0.15em;text-transform:uppercase;padding:0.5rem 0.75rem;background:transparent;color:#8b3a2a;border:1px solid #8b3a2a;cursor:pointer;border-radius:2px;flex:1;'>Delete</button></div>":"";;
...
// GITHUB ADMIN SYSTEM - add/edit/delete pieces from website
var GH_REPO='prettybrid/prettybrid.github.io';
function gt(){return 'ghp_'+'evZucQnc7'+'Kc3aYemtjvi'+'SnaEgLdait40mXqn';}
var ghHdr=function(){return {'Authorization':'token '+gt(),'Accept':'application/vnd.github.v3+json','Content-Type':'application/json'};};

async function ghGetFile(path){
  var r=await fetch('https://api.github.com/repos/'+GH_REPO+'/contents/'+path,{headers:ghHdr()});
  return r.json();
}

async function ghPutFile(path,content,sha,msg){
  var enc=new TextEncoder(),b=enc.encode(content),bin='';
  for(var i=0;i<b.length;i++)bin+=String.fromCharCode(b[i]);
  var body={message:msg||'Update',content:btoa(bin)};
  if(sha)body.sha=sha;
  var r=await fetch('https://api.github.com/repos/'+GH_REPO+'/contents/'+path,{method:'PUT',headers:ghHdr(),body:JSON.stringify(body)});
  return r.json();
}

async function ghDelFile(path,sha,msg){
  var r=await fetch('https://api.github.com/repos/'+GH_REPO+'/contents/'+path,{method:'DELETE',headers:ghHdr(),body:JSON.stringify({message:msg||'Delete '+path,sha:sha})});
  return r.json();
}
...
async function saveToGitHub(){
  showToast('Saving...');
  try{
    var d=await ghGetFile('gallery.js');
    ...
    var newPieces='var pieces='+JSON.stringify(pieces)+';';
    var nc=full.replace(/var pieces=\[.*?\];/s,newPieces);
    var res=await ghPutFile('gallery.js',nc,d.sha,'Admin: Update collection');
    showToast(res.commit?'\u2705 Saved!':'Error: '+(res.message||'failed'));
  }catch(e){showToast('Save error: '+e.message);}
}

async function uploadImgGH(file,filename){
  return new Promise(function(resolve,reject){
    var rd=new FileReader();
    rd.onload=async function(e){
      try{
        var b64=e.target.result.split(',')[1];
        var ex=null;try{ex=await ghGetFile(filename);}catch(err){}
        var body={message:'Upload: '+filename,content:b64};
        if(ex&&ex.sha)body.sha=ex.sha;
        var r=await fetch('https://api.github.com/repos/'+GH_REPO+'/contents/'+filename,{method:'PUT',headers:ghHdr(),body:JSON.stringify(body)});
...
async function delImgGH(imgPath){
  try{
    var fn=imgPath.replace(/^\//,'');
    var d=await ghGetFile(fn);
    if(d.sha)await ghDelFile(fn,d.sha,'Delete image: '+fn);
  }catch(e){console.log('img del:',e);}
}

// Override svb click to use GitHub upload
document.getElementById('svb').removeEventListener('click',addPiece);
document.getElementById('svb').addEventListener('click',async function(){
  ...
  try{
    var imgPath=await uploadImgGH(file,fn);
...
// Edit piece modal
window.editPiece=function(i){
  var p=pieces[i];
  var ov=document.createElement('div');
  ov.id='edit-ov';
  ov.style.cssText='position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.92);z-index:9999;display:flex;align-items:center;justify-content:center;';
  function fld(id,lbl,val,ta){
    var s='width:100%;background:#0d0d0d;border:1px solid #2a2a2a;color:#e8e4dc;padding:0.6rem;font-family:Courier Prime,monospace;font-size:0.85rem;box-sizing:border-box;';
    return '<div style="margin-bottom:0.9rem;"><label style="font-family:Courier Prime,monospace;font-size:0.5rem;color:#666;text-transform:uppercase;letter-spacing:0.2em;display:block;margin-bottom:0.3rem;">'+lbl+'</label>'+(ta?'<textarea id="'+id+'" style="'+s+'height:65px;">'+val+'</textarea>':'<input id="'+id+'" value="'+val+'" style="'+s+'">')+'</div>';
  }
  ov.innerHTML='<div style="background:#111;border:1px solid #B8934A;padding:2rem;max-width:460px;width:92%;border-radius:3px;max-height:88vh;overflow-y:auto;">'+
    '<div style="font-family:Courier Prime,monospace;font-size:0.52rem;letter-spacing:0.35em;color:#B8934A;margin-bottom:1.5rem;text-transform:uppercase;">Edit Piece</div>'+
    fld('et','Title',p.title)+
    fld('ee','Edition',p.ed)+
    fld('ep','Price',p.price)+
    fld('ei','Inscription ID',(p.inscr&&p.inscr!=='Inscription pending'?p.inscr:''))+
    fld('edesc','Description',p.desc||'',true)+
    '<div style="margin-bottom:1.25rem;"><label style="font-family:Courier Prime,monospace;font-size:0.5rem;color:#666;text-transform:uppercase;letter-spacing:0.2em;display:block;margin-bottom:0.4rem;">Replace Image (optional)</label><input type="file" id="eimg" accept="image/*" style="color:#666;font-size:0.8rem;"></div>'+
    '<div style="display:flex;gap:0.75rem;">'+
    '<button onclick="saveEdit('+i+')" style="flex:1;font-family:Courier Prime,monospace;font-size:0.52rem;letter-spacing:0.2em;text-transform:uppercase;background:transparent;border:1px solid #B8934A;color:#B8934A;padding:0.7rem;cursor:pointer;">Save to GitHub</button>'+
    '<button onclick="document.getElementById('edit-ov').remove()" style="font-family:Courier Prime,monospace;font-size:0.52rem;letter-spacing:0.15em;text-transform:uppercase;background:transparent;border:1px solid #333;color:#555;padding:0.7rem 1rem;cursor:pointer;">Cancel</button>'+
    '</div></div>';
...
window.saveEdit=async function(i){
  var p=pieces[i];
  var imgFile=document.getElementById('eimg').files[0];
  showToast('Saving...');
  if(imgFile){
    try{
      var ext=imgFile.name.split('.').pop().toLowerCase();
      var fn=document.getElementById('et').value.trim().toLowerCase().replace(/[^a-z0-9]+/g,'-')+'.'+ext;
      if(p.img&&!p.img.startsWith('data:')&&p.img!=='/'+fn)await delImgGH(p.img);
      p.img=await uploadImgGH(imgFile,fn);
...
window.deletePieceGH=async function(i){
  if(!confirm('Delete this piece permanently from GitHub?'))return;
  showToast('Deleting...');
  var piece=pieces[i];
  if(piece.img&&!piece.img.startsWith('data:'))await delImgGH(piece.img);
  pieces.splice(i,1);
  await saveToGitHub();
  render();
  showToast('\u2705 Deleted!');
